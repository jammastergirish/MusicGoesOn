...
$topic = $_GET['id'];
$topic_data = mysql_fetch_array(mysql_query("SELECT * FROM forum_posts WHERE id = '$topic' AND deleted = '0'"));

$topic_data[topic] = strip_tags($topic_data[topic]);

if ($forum_data = mysql_fetch_array(mysql_query("SELECT * FROM artists WHERE id = '$topic_data[forum]'")))
{
 $artist = 1;
}
else
{
 $forum_data = mysql_fetch_array(mysql_query("SELECT * FROM forums WHERE id = '$topic_data[forum]'"));
...
echo "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" style=\"table-layout:fixed\">\n\n";
 ...
 //if ($forums_row = mysql_fetch_array(mysql_query("SELECT * FROM forum_users WHERE SID = '$SID'")))
 //{
...
$posts = mysql_query("SELECT * FROM forum_posts WHERE (replyto = '$topic' OR id = '$topic') AND deleted = '0' ORDER BY id ASC");
while ($posts_data = mysql_fetch_array($posts))
{
 if (!preg_match("Guest", $posts_data[poster]))
 {
  $poster_data = mysql_fetch_array(mysql_query("SELECT * FROM forum_users WHERE username = '$posts_data[poster]'"));
 ...
 if ($forums_row = mysql_fetch_array(mysql_query("SELECT * FROM forum_users WHERE SID = '$SID'")))
 {
  mysql_query("UPDATE forum_users SET viewed = '$forums_row[viewed]|$posts_data[id]' WHERE username = '$forums_row[username]'");
 }
 else
 {
